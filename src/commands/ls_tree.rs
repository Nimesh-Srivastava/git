...
pub(crate) fn invoke(name_only: bool, tree_hash: &str) -> anyhow::Result<()> {
    let mut object = Object::read(tree_hash).context("parsing out tree object file")?;
    ...
    match object.kind {
        Kind::Tree => {
            ...
            loop {
                ...
                let mode = bits.next().expect("split always gives once");
                let name = bits
                    .next()
                    .ok_or_else(|| anyhow::anyhow!("file name missing from tree entry"))?;
                ...
                if name_only {
                    ...
                    write!(stdout, "{mode:0>6} {} {hash} ", object.kind)
                        .context("write mode, tree, and tree entry hash to stdout")?;
